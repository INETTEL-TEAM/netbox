...
- [x] **CVE/compliance** (v0.9): modelo `ComplianceFinding` (FK Device, `cve_id`, severity crit/high/med/low/info, status open/ack/remediated/false-positive, `fixed_version`, detected). Superficie completa + REST API + **GraphQL** + search + nav (grupo *Security*) + **pestaña "Compliance" en Device** (badge + Add prefilado). Migración 0008. Verificado UI/API/GraphQL/tab/search (demo: CVE-2024-6387 en core1).
- [x] **Feed CVE externo (CISA KEV)** (v0.9): `cve.py` (`fetch_kev()` del feed real CISA KEV + `sync_findings()` que empareja por **fabricante de la plataforma** y crea `ComplianceFinding` idempotente) + **JobRunner `CVESyncJob`** (programable desde NetBox→Jobs, registrado en `ready()`) + comando `inettel_sync_cve [--kev]`. **31 tests OK**, ruff limpio. Verificado: core1 (plataforma Cisco) ↔ CVE Cisco → finding creado; re-run +0.
